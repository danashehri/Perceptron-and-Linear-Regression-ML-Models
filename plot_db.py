...
def visualize_3d(df, lin_reg_weights=[1,1,1], feat1=0, feat2=1, labels=2,
    ...
    # Add scatter plot
    ax.scatter(scaling(df[feat1]), scaling(df[feat2]), df[labels])
    #ax.scatter(df[feat1], df[feat2], df[labels])

    # Set axes spacings for age, weight, height

    axes1 = scaling(np.arange(xlim[0], xlim[1], step=.05))  # age
    axes2 = scaling(np.arange(xlim[0], ylim[1], step=.05))  # weight
...
if __name__ == "__main__":
    ...
    visualize_scatter(data, weights=[-3,-2.6,6])
    # ======== SAMPLE PLOTS =======#
